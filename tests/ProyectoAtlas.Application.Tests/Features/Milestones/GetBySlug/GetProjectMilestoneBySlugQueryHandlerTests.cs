...
namespace ProyectoAtlas.Application.Tests.Features.Milestones.GetBySlug;

public class GetProjectMilestoneBySlugQueryHandlerTests
{
  [Fact]
  public async Task Execute_ShouldReturnMilestone_WhenSlugExists()
  {
    ...
    GetProjectMilestoneBySlugQueryHandler handler = new(milestoneRepository, projectRepository);

    Milestone result = await handler.Execute("proyecto-atlas", milestone.Slug);

    Assert.Same(milestone, result);
  }
  ...
  {
    Project project = CreateProject();
    GetProjectMilestoneBySlugQueryHandler handler = new(
